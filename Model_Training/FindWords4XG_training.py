...
"""
This version is for training the XGBoost model. 
It will output normalized parameters for each candidate word for human selection. 
"""
...
class FindWords4XG:
    ...
    def get_results(self, min_freq: Optional[int] = None, 
                ...
                min_entropy: Optional[float] = None) -> List[dict]:
        ...
        for word_len in range(self.config['min_word_length'], self.config['max_ngram'] + 1):
            for word, freq in self.ngram_counts[word_len].items():
                ...
                metrics.append({
                    ...
                    'tfidf': tfidf_value,
                    'hot_video_ratio':hot_video_ratio
                })
        ...
        log_freq_min = min(m['log_freq'] for m in metrics)
        log_freq_max = max(m['log_freq'] for m in metrics)
        pmi_min = min(m['pmi'] for m in metrics)
        pmi_max = max(m['pmi'] for m in metrics)
        left_ent_min = min(m['left_entropy'] for m in metrics)
        left_ent_max = max(m['left_entropy'] for m in metrics)
        right_ent_min = min(m['right_entropy'] for m in metrics)
        right_ent_max = max(m['right_entropy'] for m in metrics)
        tfidf_min = min(m['tfidf'] for m in metrics)
        tfidf_max = max(m['tfidf'] for m in metrics)

        log_freq_range = log_freq_max - log_freq_min if log_freq_max != log_freq_min else 1
        pmi_range = pmi_max - pmi_min if pmi_max != pmi_min else 1
        left_ent_range = left_ent_max - left_ent_min if left_ent_max != left_ent_min else 1
        right_ent_range = right_ent_max - right_ent_min if right_ent_max != right_ent_min else 1
        tfidf_range = tfidf_max - tfidf_min if tfidf_max != tfidf_min else 1

        
        for m in metrics:
            normalized_freq = (m['log_freq'] - log_freq_min) / log_freq_range
            normalized_pmi = (m['pmi'] - pmi_min) / pmi_range
            normalized_left_ent = (m['left_entropy'] - left_ent_min) / left_ent_range
            normalized_right_ent = (m['right_entropy'] - right_ent_min) / right_ent_range
            normalized_tfidf = (m['tfidf'] - tfidf_min) / tfidf_range

            if normalized_freq <= 0 or normalized_pmi <= 0 or normalized_left_ent <= 0 or normalized_right_ent <= 0 or normalized_tfidf <= 0:
                to_remove.add(m['word'])
                continue
            
            score = normalized_freq + 3*normalized_pmi + normalized_left_ent + normalized_right_ent + normalized_tfidf + hot_video_ratio
            ...
            candidates.append({
                ...
                'log_freq': m['log_freq'],
                'pmi': round(m['pmi'], 5),
                'left_entropy': round(m['left_entropy'], 5),
                'right_entropy': round(m['right_entropy'], 5),
                'tfidf': round(m['tfidf'], 5),
                'hot_video_ratio': round(m['hot_video_ratio'], 5),
                'NormFreq': round(normalized_freq, 5),
                'NormPmi': round(normalized_pmi, 5),
                'NormLeftEnt': round(normalized_left_ent, 5),
                'NormRightEnt': round(normalized_right_ent, 5),
                'NormTFIDF': round(normalized_tfidf, 5),
                'Score': round(score, 5),
                'Sample': self.sample_comments.get(m['word'], [])
            })
        ...
        top1000 = candidates
        logger.info("Found %d candidate new words", len(candidates))
        return top1000
